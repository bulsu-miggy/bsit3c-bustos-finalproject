         ...
         <div class="content-wrapper">
            ...
            <?php
             if(isset($_POST['submit']))
                 {$s=$_POST['title'];
                 $sql="select record.BookId,id,RollNo,Textbook,Due_Date,Date_of_Issue,datediff(curdate(),Due_Date) as x from Libraryv2.record,Libraryv2.book where (Date_of_Issue and Date_of_Return  and book.Bookid = record.BookId) and (RollNo='$s' or record.BookId='$s' or Textbook like '%$s%')";
                 }
             else
                 $sql="select record.BookId,id,RollNo,Textbook,Date_of_Return,Date_of_Issue,datediff(curdate(),Due_Date) as x from Libraryv2.record,Libraryv2.book where Date_of_Issue and Date_of_Return and book.Bookid = record.BookId";
             $result=$conn->query($sql);
             $rowcount=mysqli_num_rows($result);
             ?>

               <form action="excel2.php" method="post" style="margin-left: 35px;" class="animated pulse">
