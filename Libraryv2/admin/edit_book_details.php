   ...
   <body class="hold-transition sidebar-mini layout-fixed">
      ...
      <?php
         if (isset($_POST['submit'])) {
            ...
            // Handle image upload
            $targetDir = "../uploads/";
            $tempName = time() . "_" . $_FILES["image"]["name"];
            $targetFilePath = $targetDir . $tempName;
            $uploadOk = 1;
            ...
            if ($uploadOk == 0) {
               ...
               if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
                  $sql1 = "UPDATE Libraryv2.book SET `Section`='$Section',`Subject`='$Subject',`Textbook`='$book',`Volume`='$Title',`Year`='$Copyright',`Availability`='$availability',`Author`='$Author',`ISBN`='$ISBN',`Status`='$status',`image`='$tempName' WHERE BookId='$bookid'";
